            ...
            {allProducts.map((product, i) => (
              <tr key={i} className='border-b border-slate-900/20 text-gray-20 p-6 medium-14'>
                <td className='flexStart sm:flexCenter'>
                  <img height={43} width={43} src={product.image} alt={product.name} className="rounded-lg ring-1 ring-slate-900/5 my-1" />
                </td>
                <td>{product.name}</td>
                <td>${product.old_price}</td>
                <td>${product.new_price}</td>
                <td className='bold-20 pl-6 sm:pl-14'>{product.category}</td>
                <td>
